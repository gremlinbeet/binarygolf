...
* Skip-zero IP address, e.g. `irm 1.1`. It's effectively the same as `irm 1.0.0.1`, but shorter.
  Slightly longer variant is `irm 1.1.1`, which is the same as `irm 1.1.0.1`.
  The minimum filesize is **7** bytes, but there's only 9×10 == 90 IP addresses we can hack to install our redirect.
  With 2 zeroes skipped in the middle can also go for **8** bytes with `irm 10.1`/`irm 1.10` (90×10+9×90 == 1710 IPs), or for **9** bytes `irm 100.1`/`irm 1.100` (about 155×10+9×155 == 2945 IPs).
  The longer **9**-byte variant with single skipped zero allows for 9×9×10 == 810 IP addresses (I'm not counting the 90 IP which we can represent with 2 dot-separated digits).
...
|xref:Giving up domain name, using IP: 12 -> 11 -> 7[08]|`irm 10.1`/`irm 1.10`|Like just above, 2 zeroes in the middle ignored; 90×10+9×90 == 1710 IPs.
|xref:Giving up domain name, using IP: 12 -> 11 -> 7[09]|`irm 100.1`/`irm 1.100`|Like just above, 2 zeroes in the middle ignored; about 155×10+9×155 == 2945 IPs.
